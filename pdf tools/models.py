...
class Gap(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    page_id = db.Column(db.Integer, db.ForeignKey('whitespace.id'), nullable=False)
...
class Project(db.Model):
    ...
    def set_binarized(self, bool):
        self.is_binarized = bool
        db.session.commit()

    def set_split(self, bool):
        self.is_split = bool
        db.session.commit()

    def set_gaps(self, bool):
        self.has_gaps = bool
        db.session.commit()
...
#
#
# class Threshold(mongo.EmbeddedDocument):
...
#
#
# class Whitespace(mongo.EmbeddedDocument):
#     image_path = mongo.StringField(required=True)
#     threshold = mongo.EmbeddedDocumentField(Threshold, required=True)
...
#
#     whitespace = mongo.EmbeddedDocumentField(Whitespace)
